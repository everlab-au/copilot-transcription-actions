...
class WebmFloat extends WebmBase<number> {
  ...
  updateBySource() {
    ...
    // Create a new array with the values from byteArray
    const tempArray = Array.from(byteArray);
    const floatArray =
      floatArrayType === Float32Array
        ? new Float32Array(tempArray)
        : new Float64Array(tempArray);
...
class WebmFile extends WebmContainer {
  ...
  toBlob(type = "video/webm") {
    // Use the Uint8Array directly as BlobPart
    return new Blob([this.source!], { type });
  }
}
